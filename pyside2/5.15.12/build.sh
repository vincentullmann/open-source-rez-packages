...
mkdir build
cd build
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$PACKAGE_INSTALLATION_ROOT/$PACKAGE_NAME/$PACKAGE_VERSION -DPYTHON_EXECUTABLE=$REZ_PYTHON_ROOT/miniconda3/bin/python3 ../pyside-setup
make -j$MAKE_THREADS
...
cp bin/pyside2-uic $PACKAGE_INSTALLATION_ROOT/$PACKAGE_NAME/$PACKAGE_VERSION/bin/pyside2-uic